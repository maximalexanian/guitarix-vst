...
void GuitarixEditor::buttonClicked(juce::Button * b)
{
    ...
    else if (b == &aboutButton)
    {
        ...
        "\n"
        "For the source code see <https://github.com/maximalexanian/guitarix-vst>\n"
        "\n \n";
		...
		for (auto b = bb->begin(); b != bb->end(); ++b)
		{
			...
			if (pp)
				for (auto p = pp->begin(); p != pp->end(); ++p)
				{
					...
					if (b->get_name().raw() == bank && p->name.raw() == preset) sel = idx;
				}
			bi++;
		}
...
{
	const char* categories[] = { "Tone Control","Distortion","Fuzz","Reverb","Echo / Delay","Modulation","Guitar Effects","Misc" };
