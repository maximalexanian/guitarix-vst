...
char* ir_combo_folders[3] = { "%S", "%S/amps", "%S/bands" };
...
void JuceUiBuilder::create_ir_combo(const char *id, const char *label) {
	...
	{
		std::string path = gx_system::get_options().get_IR_prefixmap().replace_symbol(ir_combo_folders[f]);
...
void JuceUiBuilder::create_simple_meter_(const char *id) {
	//create_slider(id, "", juce::Slider::SliderStyle::LinearVertical, 20, 100);
}
